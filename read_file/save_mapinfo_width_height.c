...
/*   Updated: 2024/12/22 16:23:42 by shuga            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	check_map_helper(t_game *game, char **map, int i, int j)
{
...
void	check_map(char **map, t_game *game)
{
	...
	while (i < game->mapinfo.height - 1)
	{
		...
		{
			if (map[i][j] == '0')
				check_map_helper(game, map, i, j);
			else if (!is_space(map[i][j]) && map[i][j] != '1'
					&& map[i][j] != game->player.direction)
				goodbye(game, ERROR, "Invalid map\n");
			j++;
		}
		i++;
